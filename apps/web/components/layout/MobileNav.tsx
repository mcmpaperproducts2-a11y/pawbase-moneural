...
export function MobileNav({ user }: { user: AuthUser }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-5 border-t border-border bg-white lg:hidden">
      {links.slice(0, 4).map((module) => {
        const Icon = getModuleIcon(module.icon);
        return (
          <Link key={module.id} href={module.href} className="grid min-h-16 place-items-center gap-1 px-1 py-2 text-xs font-medium">
            <Icon size={19} />
            <span>{module.label}</span>
          </Link>
        );
      })}
      <Link href="/admin/modules" className="grid min-h-16 place-items-center gap-1 px-1 py-2 text-xs font-medium">
